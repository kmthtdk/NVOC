-- Migration: approval workflow (Phase 2)
-- Date: 2026-07-01
-- Idempotent: CREATE TABLE IF NOT EXISTS + INSERT ... ON DUPLICATE KEY UPDATE.
-- Same content ships in database/init/05_approvals.sql for fresh DBs.

-- Key/value app settings (e.g. it_leader_user_id).
...
-- Department -> leader, for resolving the 'requester_leader' approver.
...
-- Configurable approval flow, scoped globally / per category / per request_type.
...
-- Ordered steps that make up a flow (the configurable default chain).
...
-- Per-ticket materialized approval chain (one row per step, in order).
CREATE TABLE IF NOT EXISTS ticket_approvals (
  id               INT UNSIGNED NOT NULL AUTO_INCREMENT,
  ticket_id        BIGINT UNSIGNED NOT NULL,
  step_order       INT NOT NULL,
  approver_type    ENUM('requester_leader','it_leader','user','role') NOT NULL,
  approver_user_id BIGINT UNSIGNED NULL,             -- resolved approver; NULL = unassigned
  approver_label   VARCHAR(150) NULL,
  status           ENUM('pending','approved','rejected','skipped') NOT NULL DEFAULT 'pending',
  decided_by       BIGINT UNSIGNED NULL,
  decided_at       TIMESTAMP NULL,
  note             TEXT NULL,
  is_ad_hoc        TINYINT(1) NOT NULL DEFAULT 0,    -- inserted signer (open mode)
  created_at       TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
...
-- Notification outbox. Phase 3 mailing drains this; in-app reads it for now.
...
-- ---- Seed: default flow (requester_leader -> it_leader) --------------------
...
-- ---- Seed: demo leader resolution (so the default chain resolves) ----------
-- IT leader = Marcus Vance (id 2, it_support). Requester dept leader = admin (id 1).
